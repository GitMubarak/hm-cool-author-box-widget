...
if(isset($_POST['updateSocialSettings'])){
    ...
    foreach($hmcabwSocialMediaArray as $hmcabwSocial):
        $hmcabwSocialSettingsInfo['hmcabw_' . $hmcabwSocial . '_enable'] = (filter_var($_POST['hmcabw_' . $hmcabwSocial . '_enable'], FILTER_SANITIZE_NUMBER_INT)) ? $_POST['hmcabw_' . $hmcabwSocial . '_enable'] : '';
...
<div id="wph-wrap-all" class="wrap hmcabw-settings-page">
    <div class="settings-banner">
        <h2><?php esc_html_e('Author Box Social Settings', 'hm-cool-author-box-widget'); ?></h2>
    </div>
    ...
    <form name="wpre-table" role="form" class="form-horizontal" method="post" action="">
        ...
        <tr class="hmcabw_<?php echo esc_attr( $hmcabwSocialMedia); ?>_enable">
            ...
            <td>
                <input type="checkbox" name="hmcabw_<?php echo esc_attr($hmcabwSocialMedia); ?>_enable" value="1" <?php if(filter_var($hmcabwSocialSettings['hmcabw_'.$hmcabwSocialMedia.'_enable'], FILTER_SANITIZE_NUMBER_INT) == 1) { echo 'checked'; } ?>>
            </td>
        </tr>
        <tr class="hmcabw_<?php echo esc_attr( $hmcabwSocialMedia); ?>_link">
            ...
            <td>
                <input type="text" name="hmcabw_<?php echo esc_attr( $hmcabwSocialMedia); ?>_link" class="regular-text" value="<?php echo esc_url( $hmcabwSocialSettings['hmcabw_'.$hmcabwSocialMedia.'_link'] ); ?>">
            </td>
        </tr>
        <?php endforeach; ?>
        </table>
