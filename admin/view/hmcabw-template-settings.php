...
<div id="wph-wrap-all" class="wrap hmcabw-settings-page">
     <div class="settings-banner">
          <h2><?php esc_html_e('Author Box Template Settings', 'hm-cool-author-box-widget'); ?></h2>
     </div>
     <?php if($showTempMessage): $this->hmcabw_display_notification('success', 'Your information updated successfully.'); endif; ?>
    ...
    <form name="wpre-table" role="form" class="form-horizontal" method="post" action="" id="hmcabw-template-settings-form">
        ...
        <tr class="hmcabw_select_template">
            <th scope="row">
                <label for="hmcabw_select_template"><?php esc_html_e('Select a Template:', 'hm-cool-author-box-widget'); ?></label>
            </th>
            <td>
                <div class="hmcabw-template-selector">
                    <?php for($i=1; $i<7; $i++): ?>
                        <div class="hmcabw-template-item">
                            ...
                            <label for="<?php printf('hmcabw_tmp_style_%d', $i);?>" class="hmcabw-template">
                                <img src="<?php echo esc_url(HMCABW_ASSETS) ; ?><?php printf('img/temp/temp-%d', $i); ?>.png" alt="<?php printf('Default Template %d', $i); ?>" width="209">
                            </label>
                        </div>
                    <?php endfor; ?>
                </div>
        ...
        <p class="submit"><button id="updateTempSettings" name="updateTempSettings" class="button button-primary"><?php esc_attr_e('Update Template Settings', 'hm-cool-author-box-widget'); ?></button></p>
    </form>
</div>